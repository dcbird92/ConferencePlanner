...
namespace BackEnd.Endpoints
{
    public static class SessionEndpoints
    {
        ...
        {
            routes.MapGet("/api/Session/", async (ApplicationDbContext db) =>
            {
            ...
            routes.MapGet("/api/Session/{id}/", async (int id, ApplicationDbContext db) =>
            {
                return await db.Sessions.AsNoTracking()
                                .Include(s => s.Track)
                                .Include(s => s.SessionSpeakers)
            ...
            routes.MapPost("/api/Session/", async (ConferenceDTO.Session input, ApplicationDbContext db) =>
            {
            ...
            routes.MapPut("/api/Session/{id}/", async (int id, ConferenceDTO.Session input, ApplicationDbContext db) =>
            {
            ...
            routes.MapDelete("/api/Session/{id}/", async (int id, ApplicationDbContext db) =>
            {
            ...
            routes.MapPost("/api/Session/upload", async (HttpRequest req, ApplicationDbContext db) =>
            {
                if (db.Sessions.Any())
                {
                    return Results.Conflict("Session already uploaded");
                }
